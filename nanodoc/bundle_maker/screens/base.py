...
logger = get_logger("screens.base")
...
class Screen(ABC):
    ...
    def check_global_keys(self, key: int) -> bool:
        ...
        """
        logger.info(f"Checking global key: {key}")
        if key == ord("q") or key == ord("Q"):
            # Quit without confirmation
            logger.info("Quit key (q/Q) pressed")
            raise KeyboardInterrupt
        elif key == ord("h") or key == ord("H"):
            # Show help dialog
            logger.info("Help key (h/H) pressed")
            self.show_help()
            return True

        logger.debug(f"Key {key} is not a global key")
        return False
    ...
    def show_help(self) -> None:
        ...
        logger.info(f"Help screen closed with key: {key}")
