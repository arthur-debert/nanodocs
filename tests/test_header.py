...
def test_create_header_with_numerical_sequence():
    text = "Test Header"
    header = create_header(text, header_seq="numerical", seq_index=0)
    assert header == "1. Test Header"
    
    header = create_header(text, header_seq="numerical", seq_index=1)
    assert header == "2. Test Header"

def test_create_header_with_letter_sequence():
    text = "Test Header"
    header = create_header(text, header_seq="letter", seq_index=0)
    assert header == "a. Test Header"
    
    header = create_header(text, header_seq="letter", seq_index=1)
    assert header == "b. Test Header"

def test_create_header_with_roman_sequence():
    text = "Test Header"
    header = create_header(text, header_seq="roman", seq_index=0)
    assert header == "i. Test Header"
    
    header = create_header(text, header_seq="roman", seq_index=1)
    assert header == "ii. Test Header"

def test_create_header_with_filename_style():
    text = "test_file.txt"
    header = create_header(text, header_style="filename", original_path="/path/to/test_file.txt")
    assert header == "test_file.txt"

def test_create_header_with_path_style():
    text = "test_file.txt"
    header = create_header(text, header_style="path", original_path="/path/to/test_file.txt")
    assert header == "/path/to/test_file.txt"

def test_create_header_with_nice_style():
    text = "test_file.txt"
    header = create_header(text, header_style="nice", original_path="/path/to/test_file.txt")
